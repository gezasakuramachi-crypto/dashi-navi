...
| イージーカウンター | 一般画面の昨日・今日・合計アクセス数を表示 |
...
- 一般画面は運行時間外に「配信停止中」を表示し、櫻町区公会堂前に山車マークを固定表示
- 管理用ページでは公開時間外もライブ位置を確認可能
- 一般画面の最下段に昨日・今日・合計のアクセスカウンターを表示
...
一般公開ページで山車の現在地を表示する時間は次のとおりです。時間外は「配信停止中」と表示し、位置情報APIへ問い合わせず、櫻町区公会堂前に山車マークを固定表示します。
...
9月3日15:20〜17:00の経路は、暑さなどによる運行短縮に備え、通常ルートとショートカットルートを `schedule/schedule.js` に登録しています。通常時は `active: "standard"` とし、現場から切替指示があった場合だけ `active: "shortcut"` へ変更します。
