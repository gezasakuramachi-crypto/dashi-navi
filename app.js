...
window.initMap = function () {
  ...
  // 地図クリック：各 InfoWindow/パネルだけ閉じる（規制表示は維持）
  ...
  // POIマーカー（アイコン半分サイズ・クリックでInfoWindow表示）
  addCategoryMarkers(INFO_POINTS, CONFIG.ICONS.info, 20, 2500, true);
  addCategoryMarkers(WC_POINTS, CONFIG.ICONS.wc, 20, 2200, false);
  addCategoryMarkers(PARK_POINTS, CONFIG.ICONS.park, 20, 2100, false);
...
async function loadGeojsonPaths(src) {
  ...
  for (const feat of gj.features || []) {
    ...
    const t = g.type;
    if (t === "LineString") out.push({ path: toLatLngList(g.coordinates) });
    else if (t === "MultiLineString") for (const line of g.coordinates) out.push({ path: toLatLngList(line) });
    else if (t === "Polygon") out.push({ path: toLatLngList(g.coordinates[0]) });
    else if (t === "MultiPolygon") for (const poly of g.coordinates) out.push({ path: toLatLngList(poly[0]) });
  }
  loadedCache.set(src, out);
  return out;
}

/* ============ POIマーカー（クリックでInfoWindow） ============ */
function addCategoryMarkers(list, iconUrl, px = 20, zIndex = 1000, withInfo = false) {
  const baseIcon = {
    url: iconUrl,
...
function addControls() {
  ...
  const btnLocate = mk("現在地");
  const btnFollow = mk("追尾ON");
  const btnMapStyle = mk("地図:標準"); // 左下の地図切替
  ...
  btnMapStyle.onclick = () => {
    if (CURRENT_STYLE === MAP_STYLE_STANDARD) {
      ...
      btnMapStyle.textContent = "地図:すっきり";
    } else {
...
function locateOnce() {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    (e) => alert("現在地を取得できませんでした: " + e.message),
...
function showMyLocation(ll, acc = 30) {
  ...
  if (!myAccCircle) {
    ...
    myAccCircle.setRadius(Math.max(acc, 15));
  }
}
